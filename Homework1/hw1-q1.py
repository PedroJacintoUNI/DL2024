...
class LogisticRegression(LinearModel):
    def update_weight(self, x_i, y_i, learning_rate=0.001):
        ...
        ey_col = np.reshape(ey[y_i],(len(self.W),1))
        ...
        for i in range(len(self.W)):
            aux += (np.exp(self.W[i].dot(x_i)) / Z) * (np.dot(np.reshape(ey[i], (len(ey), 1)), x_i_row))

        grad = np.dot(ey_col, x_i_row) - aux
...
class MLP(object):
    ...
    def train_epoch(self, X, y, learning_rate=0.001):
        for x_i, y_i in zip(X, y):
            z1, z2, h1 = self.forward(x_i)
            grad_weights, grad_biases = self.backward(x_i,y_i,z2,h1,z1)
            self.update_parameters(grad_weights,grad_biases,learning_rate)
            

    def forward(self,x_i):
         
        z1 = (self.weights[0]).dot(x_i) + self.biases[0]
        h1 = ReLU(z1)
    ...
    def backward(self,x_i,y_i,z2,h1,z1):
        ...
        grad_weights.append(grad_z1[:, None].dot(x_i[:, None].T))
